...
def mnist_dataset(batch_size, dataset_path="./temp", val=False):
    ...
    train_dataset = tds.MNIST(root=dataset_path, train=True, download=True, transform=ttf.ToTensor())
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    if val:
        val_dataset = tds.MNIST(root=dataset_path, train=False, download=True, transform=ttf.ToTensor())
...
def cifar10_dataset(batch_size, dataset_path="./temp", val=False):
    ...
    train_dataset = tds.CIFAR10(root=dataset_path, train=True, download=True, transform=ttf.ToTensor())
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    if val:
        val_dataset = tds.CIFAR10(root=dataset_path, train=False, download=True, transform=ttf.ToTensor())
        val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=True)
        return train_dataloader, val_dataloader, labelmap
    return train_dataloader, labelmap
...
def fashion_dataset(batch_size, dataset_path="./temp", val=False):
    ...
    train_dataset = tds.FashionMNIST(root=dataset_path, train=True, download=True, transform=ttf.ToTensor())
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    if val:
        val_dataset = tds.FashionMNIST(root=dataset_path, train=False, download=True, transform=ttf.ToTensor())
        val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=True)
        return train_dataloader, val_dataloader, labelmap
    return train_dataloader, labelmap
